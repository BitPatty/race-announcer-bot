...
export class InitialMigration1628717669711 implements MigrationInterface {
  name = 'InitialMigration1628717669711';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`development\`.\`game\` (\`id\` int NOT NULL AUTO_INCREMENT, \`uuid\` char(36) NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`name\` varchar(255) NOT NULL, \`identifier\` varchar(255) NOT NULL, \`connector\` enum ('SRL', 'racetime') NOT NULL, \`abbreviation\` varchar(255) NULL, UNIQUE INDEX \`IDX_6ee95d182173a211ff700a021b\` (\`uuid\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`development\`.\`race\` (\`id\` int NOT NULL AUTO_INCREMENT, \`uuid\` char(36) NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`started_at\` datetime NULL, \`finished_at\` datetime NULL, \`goal\` varchar(255) NULL, \`url\` varchar(255) NULL, \`identifier\` varchar(255) NOT NULL, \`connector\` enum ('SRL', 'racetime') NOT NULL, \`status\` enum ('unknown', 'entry_open', 'entry_closed', 'in_progress', 'finished', 'over', 'cancelled') NOT NULL, \`last_sync_at\` datetime NULL, \`change_counter\` int NOT NULL, \`game\` int NOT NULL, UNIQUE INDEX \`IDX_b4cf9023727c1c302858d1bf32\` (\`uuid\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`development\`.\`communication_channel\` (\`id\` int NOT NULL AUTO_INCREMENT, \`uuid\` char(36) NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`is_active\` tinyint NOT NULL DEFAULT 0, \`identifier\` varchar(255) NOT NULL, \`name\` varchar(255) NULL, \`server_identifier\` varchar(255) NULL, \`type\` enum ('direct_message', 'text_channel', 'other') NOT NULL, \`permission_check_successful\` tinyint NOT NULL DEFAULT 0, \`connector\` enum ('discord') NOT NULL, UNIQUE INDEX \`IDX_bddc15eedaa6345cb741002cb9\` (\`uuid\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
